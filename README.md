...
| **Who built this**        | Non-merge commits per author with their all-history active span, and **contributor flux** — active and first-time authors per year. Recorded identities sharing a name or an email (case-insensitive) are folded into one author, labelled by the most-used name; `.mailmap` applies first and can force folds this rule cannot see |
